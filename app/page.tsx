...
export default function Page(){
 ...
 return (
 <main className="min-h-screen bg-gradient-to-b from-black via-neutral-950 to-black text-neutral-100">
  <header className="sticky top-0 z-50 border-b border-white/10 bg-black/60 backdrop-blur-xl shadow-lg shadow-black/30">
...
{/* お店タイトル掲載 */}
  <section id="home" className="max-w-6xl mx-auto px-6 pt-8 pb-20">
   ...
   <div className="relative mt-10 text-center">
    ...
    <h1 className="text-5xl md:text-7xl mt-6 tracking-[0.12em] font-semibold">coffret à bijoux</h1>
    <p className="mt-6 max-w-2xl mx-auto text-neutral-300">木漏れ日の差す静かな宝石商。八つの輝きは今宵、メスラの姿を得て舞台へ現れる。ようこそ、選ばれし客人よ。</p>
   </div>
  </section>

{/* 店員プロフィール掲載 */}
  <section id="staff" className="max-w-6xl mx-auto px-6 py-16">
   <h2 className="text-3xl mb-8 tracking-[0.14em] text-amber-200">店員 / Jewels</h2>
   <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-5">
    {gems.map(([slug,name])=>(
      <Link key={slug} href={`/cast/${slug}`} className="rounded-3xl border border-white/10 bg-black/40 backdrop-blur-md p-5 block hover:-translate-y-2 hover:shadow-2xl hover:shadow-amber-200/10 transition duration-300">
        <div className="h-40 rounded-2xl bg-gradient-to-br from-neutral-700 to-neutral-900 mb-4 flex items-center justify-center text-sm text-neutral-400 overflow-hidden">
          {slug === 'zoisite' ? (
            <img src="/kameprofile.jpg" alt={name} className="w-full h-full object-cover" />
          ) : (
            '画像追加場所'
          )}
        </div>
...
{/* ギャラリー */}
  <section id="gallery" className="max-w-6xl mx-auto px-6 py-16 pb-24">
   <h2 className="text-3xl mb-8 tracking-[0.14em] text-amber-200">Gallery</h2>
...
export default async function CastPage({params}:any){
 ...
 return (
 <main className='min-h-screen bg-gradient-to-b from-black via-neutral-950 to-black text-neutral-100 px-6 py-10'>
  <div className='max-w-5xl mx-auto'>
   <Link href='/#staff' className='text-sm text-neutral-400'>← 店員ページへ戻る</Link>
   <div className='grid md:grid-cols-2 gap-8 mt-6'>
    <div className='h-[520px] rounded-3xl border border-white/10 bg-gradient-to-br from-neutral-900 to-black flex items-center justify-center text-neutral-500'>キャスト画像追加場所</div>
    <div>
      ...
      <div className='mt-8 rounded-3xl border border-white/10 bg-black/40 backdrop-blur-md p-6 leading-8 text-neutral-300'>
